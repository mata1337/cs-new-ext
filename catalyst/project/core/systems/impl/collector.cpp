...
namespace systems {
	...
	void collector::collect_players(const std::vector<entities::cached>& raw)
	{
		...
		for (const auto& entry : raw)
		{
			...
			// Flash check: m_flFlashBangTime > current time means still flashed
			// Using m_flFlashBangTime from C_CSPlayerPawnBase
			...
			if (game_scene_node)
			{
				...
				// m_modelState + 0x80 = bone cache pointer
				...
				{
					const auto head = systems::g_bones.get(p.bone_cache).get_position(cstypes::head);
	...
	void collector::collect_items(const std::vector<entities::cached>& raw)
	{
		...
		{
			const auto view_origin = systems::g_view.origin();
			std::ranges::sort(fresh, [&view_origin](const item& a, const item& b)
				{
					return view_origin.distance(a.origin) > view_origin.distance(b.origin);
				});
		}
	...
	void collector::collect_projectiles(const std::vector<entities::cached>& raw)
	{
		...
		float current_time = 0.0f;
		{
			...
			if (gv) current_time = g::memory.read<float>(gv + 0x30);
		}
		...
		{
			const auto view_origin = systems::g_view.origin();
			std::ranges::sort(fresh, [&view_origin](const projectile& a, const projectile& b)
				{
					return view_origin.distance(a.origin) > view_origin.distance(b.origin);
				});
		}
