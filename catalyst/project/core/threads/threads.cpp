	...
	void game()
	{
		...
		while (true)
		{
			...
			if (systems::g_local.valid())
			{
				...
				// global_vars pointer: read from offset directly (not indirecting through stored pointer)
				const auto global_vars_ptr = g::memory.read<std::uintptr_t>(g::offsets.global_vars);
				if (global_vars_ptr)
				{
					// map name is at globalvars + 0x188 -> ptr -> string
					const auto map_ptr = g::memory.read<std::uintptr_t>(global_vars_ptr + 0x188);
