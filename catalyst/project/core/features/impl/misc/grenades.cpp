...
namespace features::misc {
	...
	void grenades::render_trajectory(const trajectory& traj, float alpha, std::uintptr_t weapon_hash) const
	{
		...
		for (std::size_t i = 0; i + 1 < total; ++i)
		{
			...
			zdraw::line(s0.x, s0.y, s1.x, s1.y, zdraw::rgba((int)base_color.r, (int)base_color.g, (int)base_color.b, (int)a), cfg.line_thickness);
		}
		if (cfg.show_bounces)
		{
			...
			for (const auto& bounce : traj.bounces)
			{
				...
				zdraw::rect_filled(s.x - outline_sz, s.y - outline_sz, outline_sz * 2.0f, outline_sz * 2.0f, zdraw::rgba(0, 0, 0, (int)a));
				zdraw::rect_filled(s.x - sz, s.y - sz, sz * 2.0f, sz * 2.0f, zdraw::rgba((int)cfg.bounce_color.r, (int)cfg.bounce_color.g, (int)cfg.bounce_color.b, (int)a));
			}
		}
		if (traj.end_tick >= 0)
		{
			...
			if (systems::g_view.projection_valid(s))
			{
				...
				zdraw::rect_filled(s.x - outline_sz, s.y - outline_sz, outline_sz * 2.0f, outline_sz * 2.0f, zdraw::rgba(0, 0, 0, (int)a));
				zdraw::rect_filled(s.x - sz, s.y - sz, sz * 2.0f, sz * 2.0f, zdraw::rgba((int)cfg.detonate_color.r, (int)cfg.detonate_color.g, (int)cfg.detonate_color.b, (int)a));
			}
		}
	}
} // namespace features::misc