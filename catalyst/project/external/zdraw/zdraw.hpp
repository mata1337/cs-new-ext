...
namespace zdraw {
	...
	struct rgba
	{
		...
		constexpr rgba(int r_, int g_, int b_, int a_) : r{ static_cast<std::uint8_t>(r_) }, g{ static_cast<std::uint8_t>(g_) }, b{ static_cast<std::uint8_t>(b_) }, a{ static_cast<std::uint8_t>(a_) } {}
	...
	public:
		...
		[[nodiscard]] std::size_t size() const noexcept { return this->m_size; }
		[[nodiscard]] const T* data() const noexcept { return this->m_data.data(); }
		[[nodiscard]] T* data() noexcept { return this->m_data.data(); }
		[[nodiscard]] std::span<const T> span() const noexcept { return std::span{ this->m_data.data(), this->m_size }; }
	};
	struct draw_list
	{
		...
		void push_vertex(float x, float y, float u, float v, rgba color)
		{
			...
			vtx->m_pos[0] = x; vtx->m_pos[1] = y;
			vtx->m_uv[0] = u; vtx->m_uv[1] = v;
			vtx->m_col = color;
		}
	...
	struct glyph_cache_entry
	{
		...
		float m_quad_x0{ 0.0f }; float m_quad_y0{ 0.0f };
		float m_quad_x1{ 0.0f }; float m_quad_y1{ 0.0f };
		float m_uv_x0{ 0.0f };   float m_uv_y0{ 0.0f };
		float m_uv_x1{ 0.0f };   float m_uv_y1{ 0.0f };
		bool m_valid{ false };
	};
	struct font
	{
		...
		{
			float m_advance_x{ 0.0f };
			float m_bearing_x{ 0.0f }; float m_bearing_y{ 0.0f };
			float m_width{ 0.0f };     float m_height{ 0.0f };
			float m_atlas_x{ 0.0f };   float m_atlas_y{ 0.0f };
		};
	...
	void text(float x, float y, std::string_view str, rgba color, const font* fnt = nullptr)
	{
		...
		else if constexpr (std::is_same_v<style, tstyles::outlined>)
		{
			...
			for (int i{ 0 }; i < 4; ++i)
				get_draw_list().add_text(x + offsets[i][0], y + offsets[i][1], str, f, rgba(0, 0, 0, (int)color.a));
			get_draw_list().add_text(x, y, str, f, color);
		}
		else if constexpr (std::is_same_v<style, tstyles::shadowed>)
		{
			constexpr float offset{ 1.0f };
			get_draw_list().add_text(x + offset, y + offset, str, f, rgba(0, 0, 0, (int)color.a));
			get_draw_list().add_text(x, y, str, f, color);
		}
		else
		{
